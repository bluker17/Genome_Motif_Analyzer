...
class Motifs:
    def __init__(self, motif_info, genomes):
        self.motif_info = motif_info
        self.genomes = genomes
        self.motif_results = {}


    def stream_fasta(file: Path):
        """
    ...
    def array_set(self, genome: str):
        '''
        Purpose
        -------
        Transforms the genome sequence into a Numpy array.
        ...
        '''
        self.genome = np.array(list(genome))
    ...
    def motif_search(self, motif: str) -> np.ndarray:
        ...
        # Step 2
        windows = sliding_window_view(self.genome, len(motif))
    ...
    def get_motif_results(self):
        ...
        for genome in self.genomes:
            for chrom_name, sequence in self.stream_fasta(genome):
                self.array_set(sequence)

                for enzyme in self.motif_info:
                    motif = self.motif_info[enzyme]["motif_sequence"]
                    positions = self.motif_search(motif)
                    count = len(positions)
                    # self.motif_results[enzyme] = [motif, count, positions.tolist()]


                    self.motif_results.setdefault(enzyme, [])
                    self.motif_results[enzyme].append({
